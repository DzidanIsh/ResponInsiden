...
class EradicationManager:
    def __init__(self):
        ...
        self.staging_dir = "/tmp/scan_staging"
    ...
    def _check_clamav_availability(self):
        ...
        try:
            ...
            for socket_path in possible_socket_paths:
                try:
                    logger.info(f"Mencoba menghubungkan ke ClamAV daemon di {socket_path}")
                    cd = pyclamd.ClamdUnixSocket(socket_path)  # Gunakan socket_path sebagai parameter
                    ping_result = cd.ping()
                    if ping_result:
                        self.clamd_client = cd
                        self.clamd_socket_path = socket_path  # Simpan path yang berhasil
    ...
    def scan_with_clamav(self, file_path):
        if not self.clamav_enabled:
            logger.warning("ClamAV dinonaktifkan. Melewati scan.")
            return None
        if not hasattr(self, 'clamd_client'):
            logger.error("ClamAV client tidak tersedia. Melewati scan.")
            return None

        staging_path = None

        try:
            logger.info(f"Memulai scan ClamAV untuk file '{file_path}'")

            if not os.path.exists(file_path):
                logger.error(f"File '{file_path}' tidak ditemukan untuk scan ClamAV")
                return None
            ...
            try:
                ...
                logger.warning(f"Tidak dapat membuka file langsung, mencoba menyalin ke staging: {e}")
                try:
                    staging_path = os.path.join(self.staging_dir, os.path.basename(file_path))
                    shutil.copy(file_path, staging_path)
                    os.chmod(staging_path, 0o644)
                    logger.info(f"File disalin ke staging path '{staging_path}' untuk pemindaian")
                except Exception as copy_err:
                    logger.error(f"Gagal menyalin file ke staging: {copy_err}")
                    return None

            path_to_scan = staging_path if staging_path else file_path
            result = self.clamd_client.scan_file(path_to_scan)
            logger.info(f"Hasil scan ClamAV: {result}")

            if result:
                status, virus_name = list(result.values())[0]
                if status == 'FOUND':
                    logger.warning(f"ClamAV FOUND threat: '{virus_name}' di file '{path_to_scan}'")
                    return {'status': 'FOUND', 'details': virus_name}
                elif status == 'ERROR':
                    logger.error(f"Error ClamAV saat scan file '{path_to_scan}': {virus_name}")
                    return None
                else:
                    logger.info(f"ClamAV scan OK untuk file '{path_to_scan}'")
            return {'status': 'OK', 'details': None}
        except Exception as e:
            logger.error(f"Error saat scan ClamAV via pyclamd: {e}", exc_info=True)
            return None
        finally:
            if staging_path and os.path.exists(staging_path):
                try:
                    os.remove(staging_path)
                    logger.info(f"File staging '{staging_path}' telah dihapus setelah scan.")
                except Exception as cleanup_err:
                    logger.warning(f"Gagal menghapus file staging '{staging_path}': {cleanup_err}")
    ...
    def _perform_all_scans_on_file(self, file_path):
        ...
        yara_matches_obj = self.scan_with_yara(file_path)
        if yara_matches_obj:
            rule_names = list(set([match.rule for match in yara_matches_obj]))
            return True, f"YARA({','.join(rule_names)})"
